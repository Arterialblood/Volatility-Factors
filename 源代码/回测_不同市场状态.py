...
def analyze_all_factors():
    ...
    对所有因子进行分析并生成报告
    """
    def define_market_states():
        """定义市场状态"""
        market_states = pd.DataFrame(index=pd.date_range('2021-01-01', '2024-11-01'))
        
        # 定义各个时期的市场状态
        state_periods = [
            ('2021-01-01', '2021-01-31', 'bull'),
            ('2021-02-01', '2022-04-30', 'bear'),
            ('2022-05-01', '2022-06-30', 'bull'),
            ('2022-07-01', '2022-10-31', 'bear'),
            ('2022-11-01', '2023-01-31', 'bull'),
            ('2023-02-01', '2024-01-31', 'bear'),
            ('2024-02-01', '2024-06-30', 'oscillation'),
            ('2024-07-01', '2024-09-23', 'bear'),
            ('2024-09-24', '2024-11-01', 'bull')
        ]
        
        market_states['state'] = None
        for start, end, state in state_periods:
            mask = (market_states.index >= start) & (market_states.index <= end)
            market_states.loc[mask, 'state'] = state
        
        return market_states

    # 获取市场状态
    market_states = define_market_states()
    
    # 创建回测系统
    backtest = BacktestSystem(integrated_data)
    
    # 分析指定的因子
    factors_to_analyze = ['VOL_3M', 'EWMAVOL', 'GARCHVOL']
    all_results = []
    
    for factor_name in factors_to_analyze:
        print(f"\n分析因子: {factor_name}")
        factor_values = backtest.preprocessor.process_factor(factor_name)
        results = []
        
        for state in ['bull', 'bear', 'oscillation']:
            state_dates = market_states[market_states['state'] == state].index
            state_dates = state_dates.intersection(backtest.monthly_dates)
            
            if len(state_dates) < 2:
                continue
            ...
            for i in range(len(state_dates)-1):
                current_date = state_dates[i]
                next_date = state_dates[i+1]
                
                if current_date in factor_values.index:
                    current_factors = factor_values.loc[current_date]
                    
                    mask = (integrated_data.get_stock_returns().index > current_date) & \
                          (integrated_data.get_stock_returns().index <= next_date)
                    next_month_returns = integrated_data.get_stock_returns()[mask].mean()
            ...
            rankic_positive_ratio = (rank_ic_series > 0).mean()
            
            # 年化处理
            annual_ic_ir = ic_ir * np.sqrt(12)
            annual_rankic_ir = rankic_ir * np.sqrt(12)
            ...
            results.append({
                ...
                'rankIC均值': rankic_mean,
                'rankIC标准差': rankic_std,
                '年化rankICIR': annual_rankic_ir,
                'rankIC>0占比': rankic_positive_ratio
            })
    ...
    output_path = '因子在不同市场状态下的表现.xlsx'
