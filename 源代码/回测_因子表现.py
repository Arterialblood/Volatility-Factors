        ...
        for i in range(len(self.monthly_dates)-1):
            ...
            if current_date in factor_values.index:
                ...
                if len(common_stocks) > 0:
                    ...
                    print(f"收益��名范围: {next_month_returns[common_stocks].rank().min():.0f} 到 {next_month_returns[common_stocks].rank().max():.0f}")
...
class ResultAnalyzer:
    ...
    def generate_performance_table(self, returns):
        """生成因子收益表现表格"""
        # 多头表现
    ...
    def generate_factor_analysis_report(self, factor_name, output_path=None):
        ...
        # 直接使用self.results而不是self.results['raw_results']
        group1_returns = self.results['returns']['Group_1']
        group1_perf = self.calculate_performance_metrics(group1_returns)
        group1_excess = self.calculate_excess_metrics(group1_returns)
        ...
        results = {
            ...
            '因子IC分析': {
                'IC均值': f"{self.results['ic'].mean():.4f}",
                'IC标准差': f"{self.results['ic'].std():.4f}",
                '年化ICIR': f"{(self.results['ic'].mean()/self.results['ic'].std()) * np.sqrt(12):.4f}",
                'RankIC均值': f"{self.results['rank_ic'].mean():.4f}",
                'RankIC标准差': f"{self.results['rank_ic'].std():.4f}",
                '年化RankICIR': f"{(self.results['rank_ic'].mean()/self.results['rank_ic'].std()) * np.sqrt(12):.4f}",
                'RankIC>0占比': f"{(self.results['rank_ic'] > 0).mean():.2%}"
            }
        }
...
def analyze_all_factors():
    ...
    for factor_name in all_factor_names:
        ...
        # 生成报告
        output_path = f'factor_analysis_{factor_name}.xlsx'
        analyzer.generate_factor_analysis_report(factor_name, output_path)
        
        # 打印简要结果
        analyzer.print_factor_analysis_results()
        
        print(f"\n{factor_name}因子分析报告已保存至: {output_path}")
